...
void PatientInfoManager::searchDataSended(QString id, QString data)
{
    ...
    pixmap->scaled(200,180,Qt::KeepAspectRatioByExpanding);
...
void PatientInfoManager::on_modifyPushButton_clicked()
{
    ...
    if(ui->clientInfoTableWidget->item(2,0)->text() != "여성" || ui->clientInfoTableWidget->item(2,0)->text() != "남성")
    {
        QMessageBox::critical(qmsgBox, tr("경고"), tr("여성/남성 중 하나의 성별을 선택해 수정해주세요."));
        ui->clientInfoTableWidget->item(2, 0)->setText(sex);
        return;
    }
